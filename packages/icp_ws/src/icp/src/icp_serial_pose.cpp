...
Eigen::MatrixXd icp_serial(int n_itr, Eigen::Matrix2d &R_tot, Eigen::Vector2d &t_tot) {
...
geometry_msgs::Transform get_transform_msg(Eigen::Matrix2d R, Eigen::Vector2d t) {
    geometry_msgs::Transform tf;
    float yaw = std::atan2(R.row(1)[0], R.row(0)[0]);
    
    tf.rotation.x = 0;
    tf.rotation.y = 0;
    tf.rotation.z = std::sin(yaw/2);
    tf.rotation.w = std::cos(yaw/2);

    tf.translation.x = t[0];
    tf.translation.y = t[1];
    tf.translation.z = 0;
...
int main(int argc, char** argv)
{
    ...
    // Publisher of Transform
    ros::Publisher transform_pub = nh.advertise<geometry_msgs::Transform>("transform", 10);
    ...
    while (ros::ok()) {
        if (!(prev_pc.points.empty()) && !(curr_pc.points.empty())) {
            ...
            Eigen::Matrix2d R_tot = Eigen::Matrix2d::Identity(2,2);
            Eigen::Vector2d t_tot = Eigen::Vector2d::Zero();
            clock_gettime(CLOCK_MONOTONIC, &start_time);
            P = icp_serial(icp_itr, R_tot, t_tot);
            ...
            transform_msg = get_transform_msg(R_tot, t_tot);
